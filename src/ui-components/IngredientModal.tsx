...
import { addIngredient, updateIngredient } from '../utils/ingredientSlice';
import getNewViewOrderIngredient from '../utils/getNewViewOrderIngredient';

function IngredientModal({ ingredient, open, handleClose, isAdd, recipeName }: { ingredient: Ingredient, open: boolean, handleClose: () => void, isAdd: boolean, recipeName: string }) {
    const ingredients: any = useAppSelector((state) => state.ingredients.ingredients);
    const initialState = {
        id: { value: ingredient.id },
        view_order: { value: ingredient.view_order ? ingredient.view_order : 0 },
        ingedient: { value: ingredient.title, touched: false, hasError: true, error: "" },
        thumbnail: { value: ingredient.thumbnail, touched: false, hasError: true, error: "" },
        description: { value: ingredient.description, touched: false, hasError: true, error: "" },
    ...
    const updateForm = async (e: any) => {
        ...
        if (!isFormValid) {
            ...
            const data: Ingredient = {
                id: isAdd ? getNewViewOrderIngredient(ingredients) : formState.id.value,
                view_order: isAdd ? getNewViewOrderIngredient(ingredients) : formState.view_order.value,
                title: formState.ingedient.value,
                thumbnail: formState.thumbnail.value,
                description: formState.description.value,
                recipe_name: formState.recipe_name.value,
            }
            const responseData = isAdd ? await ingredientApiService.addIngredient(data) : await ingredientApiService.updateIngredient(data);
            isAdd ? dispatch(addIngredient(responseData?.data?.insertingredients?.value)) : dispatch(updateIngredient(responseData?.data?.updateingredients?.value));
            setIsLoading(false);
            toastService({ text: `Ingredient ${isAdd ? 'added' : 'updated'}`, toastType: ToastType.Success });
            handleClose();
        }
    }
    return (
                ...
                <Box sx={style}>
                    <FormContainer onSubmit={e => updateForm(e)}>
                        ...
                        <AddRecipeContainer>
                            <FormInputSet>
                                <Label label="Recipe" />
                                <Input disabled className={formState.recipe_name.touched && formState.recipe_name.hasError && `error`} type="text" value={formState.recipe_name.value} onChange={e => {
                            ...
                            <FormInputSet>
                                <Label label="Ingedient" />
                                <Input className={formState.ingedient.touched && formState.ingedient.hasError && `error`} type="text" value={formState.ingedient.value} onChange={e => {
                                    onInputChange("ingedient", e.target.value, formdispatch, formState)
                                }} onBlur={e => {
                                    onFocusOut("ingedient", e.target.value, formdispatch, formState)
                                }} />
                                {formState.ingedient.touched && formState.ingedient.hasError && (
                                    <ErrorMsg className="error">{formState.ingedient.error}</ErrorMsg>
                                )}
                            </FormInputSet>
                            <FormInputSet>
